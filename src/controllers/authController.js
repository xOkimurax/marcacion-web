...
import { signToken, verifyToken as verifyJWT } from '../utils/jwt.js';
...
// Genera la URL de OAuth y devuelve también un pkceToken (JWT con codeVerifier)
// El frontend lo guarda en sessionStorage y lo manda al hacer exchange
export async function getGoogleOAuthUrl(req, res) {
  try {
    ...
    // Firmar el codeVerifier como JWT para que el frontend lo guarde y lo mande de vuelta
    const pkceToken = signToken({ codeVerifier, codeChallenge }, '15m');

    return res.json({ url: finalUrl, pkceToken });
  } catch (error) {
...
export async function exchangeCode(req, res) {
  try {
    const { code, pkceToken } = req.body;
    if (!code) return res.status(400).json({ error: 'code is required.' });
    if (!pkceToken) return res.status(400).json({ error: 'pkceToken is required.' });

    // Extraer codeVerifier del pkceToken firmado
    let codeVerifier;
    try {
      const decoded = verifyJWT(pkceToken);
      codeVerifier = decoded.codeVerifier;
    } catch {
      return res.status(400).json({ error: 'pkceToken invalid or expired. Please try again.' });
    }
    ...
    const insforgeToken = exchangeData.access_token || exchangeData.token || exchangeData.accessToken;
    if (!insforgeToken) {
...
export async function verifyInsforgeToken(req, res) {
  try {
    ...
    const meRes = await fetch(`${INSFORGE_URL}/api/auth/me`, {
      headers: { 'Authorization': `Bearer ${token}`, 'x-api-key': INSFORGE_API_KEY },
    });
    ...
    if (!user) {
      const created = await pool.query(
        `INSERT INTO users (insforge_id, email, name, picture, role, is_active) VALUES ($1,$2,$3,$4,'EMPLOYEE',true) RETURNING *`,
        [insforgeId, email, name || email, picture]
      );
      user = created.rows[0];
    } else {
      const updated = await pool.query(
        `UPDATE users SET insforge_id=$1, name=$2, picture=$3, updated_at=NOW() WHERE id=$4 RETURNING *`,
...
export async function googleLogin(req, res) {
  req.body.token = req.body.token || req.body.insforgeToken;
  return verifyInsforgeToken(req, res);
}
